<?php
    ...
    if(isset($_POST['upd_contact'])){
        $frm_data = filteration($_POST);
        $q = "UPDATE `contact_detail` SET `address`=?,`gmap`=?,`pn1`=?,`pn2`=?,`email`=?,`fb`=?,`ins`=?,`tw`=?,`tt`=?,`iframe`=? WHERE `sr_no`=?";
        $values = [$frm_data['address'],$frm_data['gmap'],$frm_data['pn1'],$frm_data['pn2'],$frm_data['email'],$frm_data['fb'],$frm_data['ins'],$frm_data['tw'],$frm_data['tt'],$frm_data['iframe'],1];
